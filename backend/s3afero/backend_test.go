...
func TestPutGet(t *testing.T) {
	...
	for _, backend := range backends {
		t.Run("", func(t *testing.T) {
			...
			if _, err := backend.PutObject("test", "yep", meta, bytes.NewReader(contents), int64(len(contents)), "STANDARD"); err != nil {
				t.Fatal(err)
			}
			hasher := md5.New()
...
func TestPutListRoot(t *testing.T) {
	...
	for _, backend := range backends {
		t.Run(fmt.Sprintf("%T", backend), func(t *testing.T) {
			...
			if _, err := backend.PutObject("test", "foo", meta, bytes.NewReader(contents1), int64(len(contents1)), "STANDARD"); err != nil {
				t.Fatal(err)
			}

			contents2 := []byte("contents2")
			if _, err := backend.PutObject("test", "bar", meta, bytes.NewReader(contents2), int64(len(contents2)), "STANDARD"); err != nil {
				t.Fatal(err)
			}
...
func TestPutListDir(t *testing.T) {
	...
	for _, backend := range backends {
		t.Run(fmt.Sprintf("%T", backend), func(t *testing.T) {
			...
			contents1 := []byte("contents1")
			if _, err := backend.PutObject("test", "foo/bar", meta, bytes.NewReader(contents1), int64(len(contents1)), "STANDARD"); err != nil {
				t.Fatal(err)
			}

			contents2 := []byte("contents2")
			if _, err := backend.PutObject("test", "foo/baz", meta, bytes.NewReader(contents2), int64(len(contents2)), "STANDARD"); err != nil {
				t.Fatal(err)
			}
...
func TestPutDelete(t *testing.T) {
	...
	for _, backend := range backends {
		t.Run(fmt.Sprintf("%T", backend), func(t *testing.T) {
			...
			if _, err := backend.PutObject("test", "foo", meta, bytes.NewReader(contents), int64(len(contents)), "STANDARD"); err != nil {
				t.Fatal(err)
			}
